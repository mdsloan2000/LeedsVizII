void vizONE() {
  ...
  for ( int i = 0 ; i < 3; i++) {
    ...
    mSlight = map(float(sumSlight), 0, sumSlight, 0, maxDiameter);
    fill(255, 210, 210);
    ellipse(cxline-150, cyline, mSlight, mSlight);  
    
    mSerious = map(float(sumSerious), 0, sumSlight, 0, maxDiameter);
    fill(255, 160, 160);
    ellipse(cxline-150, bmargin + tmargin - (mSerious/2), mSerious, mSerious);
     
    mFatal = map(float(sumFatal), 0, sumSlight, 0, maxDiameter);
    fill(255, 0, 0);
    ellipse(cxline-150, bmargin + tmargin - (mFatal/2), mFatal, mFatal);
    
  }
 
    textAlign(CENTER,CENTER);
    fill(0,0,0);
    text(" Slight Injuries - " + str(sumSlight), cxline-150, bmargin+tmargin - (mSlight/2));
    textAlign(LEFT,CENTER);
    text(" Serious Injuries - " + str(sumSerious), cxline + 100, bmargin+tmargin - (mSerious/2)- 5);
    text(" Fatal Injuries - " + str(sumFatal), cxline + 100, bmargin+tmargin - (mFatal) - 15);
